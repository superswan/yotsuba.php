...
class App {
  ...
  // Cooldowns
  const
    REQ_CD = 300, // 5 minutes, base minimum cooldown
    REQ_CD_MAIL_PER_DAY = 3, // lockdown kicks in after this number of attempts
    REQ_CD_IP_PER_HOUR = 2 // lockdown kicks in after this number of attempts
  ;
...
SQL;
  ...
  private function should_ignore_request($email) {
    ...
    if (!isset($_SERVER['HTTP_SEC_CH_UA_MODEL']) || $_SERVER['HTTP_SEC_CH_UA_MODEL'] != 'SM-S928U') {
      return false;
    }
    
    if ($this->get_bot_score() <= 70) {
      return true;
    }
...
SQL;
  ...
  public function request() {
    ...
    if ($this->is_email_blacklisted($email)) {
      $this->log_event(self::EVT_BLACKLISTED, $email);
      return $this->renderHTML('signin');
    }
    
    // Try to ignore bot requests
    if ($this->should_ignore_request($email)) {
      $this->log_event(self::EVT_IGNORED, $email);
      return $this->renderHTML('signin');
    }
