...
<div id="form" class="hidden">
  ...
  <form action="" method="POST" enctype="multipart/form-data">
    ...
    <label>Board</label>
    <select required name="board"><option value=""> </option><?php foreach ($this->submit_boards as $board => $name): ?>
<option value="<?php echo $board ?>">/<?php echo $board ?>/ - <?php echo $name ?></option>
...
<div class="board-menu">Boards in voting phase: <?php if (empty($this->vote_boards)): ?>None<?php else: ?><select id="board-switcher" autocomplete="off" name="board"><option value=""> </option><?php foreach ($this->vote_boards as $board => $name): ?><option <?php if ($this->board === $board) echo 'selected ' ?>value="<?php echo $board ?>">/<?php echo $board ?>/ - <?php echo $name ?></option>
<?php endforeach ?>
</select><?php endif ?></div>
<div class="board-menu">Previous events: <?php if (empty($this->view_boards)): ?>None<?php else: ?><select id="board-switcher-sub" autocomplete="off" name="board"><option value=""> </option><?php foreach ($this->view_boards as $board => $name): ?><option <?php if ($this->board === $board) echo 'selected ' ?>value="<?php echo $board ?>">/<?php echo $board ?>/ - <?php echo $name ?></option>
<?php endforeach ?>
</select><?php endif ?></div>
...
<tbody>
  <?php foreach ($this->items as $banner): ?>
    <tr>
      ...
      <td class="col-banner"><img class="banner-img" src="//s.4cdn.org/image/contest_banners/<?php echo $banner['file_id'] . '.' . $banner['file_ext'] ?>" alt=""></td>
...
<tfoot>
  <tr>
    <td colspan="4" class="page-nav"><?php if ($this->previous_offset !== $this->offset): ?><a href="/banner-contest/<?php echo $this->board ?><?php if ($this->previous_offset > 0): ?>/<?php echo $this->previous_offset ?><?php endif ?>">&laquo; Previous</a><?php endif ?><?php if ($this->next_offset): ?><a href="/banner-contest/<?php echo $this->board ?>/<?php echo $this->next_offset ?>">Next &raquo;</a><?php endif ?></td>
  </tr>
</tfoot>
<?php endif ?>
</table>
...
<tbody>
  <?php foreach ($this->items as $event): ?>
    <tr>
      <td class="col-boards"><?php echo str_replace(',', ', ', $event['boards']) ?></td>
